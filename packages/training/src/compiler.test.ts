...
it("rejects a silent assert-only suite even when every exit code is correct",async()=>{
  const {report}=await compileQuestion(design,async(files)=>({
    exitCode:files["queue.ts"]==="incorrect"&&"hidden.test.ts" in files?1:0,
    stdout:"",stderr:"",durationMs:5,
  }));
  expect(report.valid).toBe(false);
  expect(report.checks.find(check=>check.name==="visible case results")?.detail).toContain("silent assert-only tests");
