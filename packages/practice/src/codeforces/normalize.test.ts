...
<div class="problem-statement">
  ...
  <div><p>Given a watermelon of weight <span class="tex-span">w</span>, decide whether it can be split.</p></div>
...
describe("Codeforces normalization", () => {
  ...
  it("extracts the statement and complete stdin/stdout examples", () => {
    ...
    expect(problem?.examples).toEqual([{ input: ["8"], output: "YES", explanation: "" }]);
    expect(casesForCodeforcesProblem(problem!)).toEqual([{ name: "Example 1", input: ["8"], expected: "YES", origin: "source" }]);
