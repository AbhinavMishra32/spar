...
export function installRoutes(app: FastifyInstance, db: Database) {
  ...
  app.post("/v1/sessions", async (request,reply) => { const user=await requireUser(request); const goal=String((request.body as {goal?:unknown}).goal??"").trim(); if(goal.length<3)return reply.code(400).send({error:"Goal is required"}); const id=randomUUID(); await db.insert(sessions).values({id,userId:user.id,title:goal.slice(0,80),originalGoal:goal}); return reply.code(201).send({id}); });
