...
export async function createServer(environment=process.env){const env=envSchema.parse(environment);const app=Fastify({logger:{level:env.NODE_ENV==="production"?"info":"debug"},requestIdHeader:"x-request-id",trustProxy:true});const database=createDatabase(env.DATABASE_URL);await app.register(cors,{origin:false});await app.register(jwt,{secret:env.AUTH_SECRET});installAuth(app,database.db);installRoutes(app,database.db);app.addHook("onClose",()=>database.close());return {app,env};}
