...
export function installAuth(app: FastifyInstance, db: Database) {
  ...
  app.post("/v1/auth/exchange", async (request, reply) => {
    const body = request.body as Record<string,string>; const entry = pending.get(body.code); if (!entry || entry.expiresAt < Date.now() || entry.state !== body.state || !verifyPkce(body.verifier, entry.challenge)) return reply.code(401).send({ error: "Invalid or expired authorization code" });
    pending.delete(body.code); const rows = await db.select().from(users).where(eq(users.id, entry.userId)).limit(1); const user = rows[0]; if (!user) return reply.code(401).send({ error: "Account no longer exists" });
