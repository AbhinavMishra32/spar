 ...
 * path, monochrome, inheriting the row's colour. All three are knockout marks —
 * a filled square or hex with the letterforms punched out — so they read as one
 * family at 14px even though they come from three different brands.
 *
...
export function LanguageGlyph({ className, language, ...props }: GlyphProps & { language: Language }) {
  return (
    <svg
      focusable="false"
      role="img"
    ...
    >
      <path d={PATHS[language]} fill="currentColor" />
    </svg>
  );
}
