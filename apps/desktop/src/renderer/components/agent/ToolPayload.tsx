...
function Payload({ title, body }: { title: string; body: string }) {
  ...
  return (
    <div className="min-w-0">
      <p className="px-2.5 pt-2 pb-1 text-ui-sm font-medium tracking-[0.12em] text-muted-foreground/70 uppercase">{title}</p>
      <FadedScroll uncapped={full} watch={body}>
        <pre className="px-2.5 pb-2 font-mono text-ui-sm leading-[1.45] whitespace-pre text-muted-foreground/80">{trimmed}</pre>
      </FadedScroll>
      {lines > 8 && (
        <button
          className="mx-2.5 mb-2 cursor-default rounded-md bg-[color-mix(in_oklab,var(--foreground)_5%,transparent)] px-2 py-1 text-ui-sm text-muted-foreground transition-colors hover:text-foreground"
