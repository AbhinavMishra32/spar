...
export function ArtifactCardRow({ icon, children, compact = false, className }: { icon: ReactNode; children: ReactNode; compact?: boolean; className?: string }) {
  return (
    <div className={cn("flex min-w-0 items-center", compact ? "gap-1.5 px-1.5 py-1" : "gap-2.5 px-2.5 py-1.5", className)}>
      <span className={cn("grid shrink-0 place-items-center rounded-[var(--radius-md)] bg-[var(--color-background-elevated-secondary)] ring-[0.5px] ring-[var(--border-surface-strong)]", compact ? "size-5" : "size-7")}>{icon}</span>
      {children}
    </div>
  );
}
