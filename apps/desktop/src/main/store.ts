...
export class LocalStore {
  ...
  getBaseline():BaselineState{return baselineStateSchema.catch({status:"not-started",confidence:0,directEvidenceCount:0,importedEvidenceCount:0,completedAt:null}).parse(this.getSetting("baseline-state",{status:"not-started",confidence:0,directEvidenceCount:0,importedEvidenceCount:0,completedAt:null}));}
  ...
  private backfillTracks(){const sessions=this.db.prepare("SELECT id,title,original_goal,created_at,updated_at FROM sessions WHERE track_id IS NULL").all() as Array<{id:string;title:string;original_goal:string;created_at:string;updated_at:string}>;const insert=this.db.prepare("INSERT OR IGNORE INTO tracks (id,title,goal,status,emphasis,priorities,investigating,monitoring,created_at,updated_at) VALUES (?,?,?,'active','[]','[]','[]','[]',?,?)");const attach=this.db.prepare("UPDATE sessions SET track_id=? WHERE id=?");this.db.transaction(()=>{for(const session of sessions){const trackId=randomUUID();insert.run(trackId,session.title.slice(0,80),session.original_goal,session.created_at,session.updated_at);attach.run(trackId,session.id);}})();if(!this.getSetting<string>("active-track-id","")&&sessions.length){const row=this.db.prepare("SELECT track_id FROM sessions ORDER BY updated_at DESC LIMIT 1").get() as {track_id:string}|undefined;if(row?.track_id)this.setSetting("active-track-id",row.track_id);}}
