...
function route(kind:string,value:Record<string,unknown>):{path:string;method:string;body:unknown}|null{if(kind==="session-create")return{path:"/v1/sessions",method:"POST",body:{id:value.sessionId,goal:value.goal,title:value.title}};if(kind==="checkpoint")return{path:`/v1/sessions/${value.sessionId}/checkpoints/${value.version}`,method:"PUT",body:value};if(kind==="attempt-event")return{path:`/v1/attempts/${value.attemptId}/events`,method:"POST",body:{attemptId:value.attemptId,expectedSequence:value.sequence,events:[value]}};return null;}
