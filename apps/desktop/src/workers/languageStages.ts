...
export function resolveLanguageStages(root:string,language:Language,command:"test"|"run"):StageResolution{
  ...
  if(language==="go")return tests("**/*_test.go").length?{stages:[{bin:"go",args:["test","./..."]}]}:{error:"No Go test sources found. Use *_test.go files.\n"};
  ...
  if(language==="rust"){const testFiles=tests("**/{*_test.rs,*.test.rs}");if(!testFiles.length)return{error:"No Rust test sources found. Use *_test.rs or *.test.rs files.\n"};return{stages:testFiles.flatMap((file,index)=>{const binary=path.join(output,`rust-test-${index}`);rmSync(binary,{force:true});return[{bin:"rustc",args:["--edition=2021","--test",file,"-o",binary]},{bin:binary,args:[]}];})};}
