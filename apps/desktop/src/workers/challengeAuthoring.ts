...
- Write actual line breaks in the statement value, never literal backslash-n separators. Keep the opening to two or three short sentences and put each remaining rule on its own line.
- For a learner new to the subject, test one idea from the lesson they were given. Define specialist terms with a concrete example before asking the learner to use them. Describe a baseline or a small workflow step accurately; do not present it as training a complete supervised model. Avoid repeating the same rule in the description, constraints, and solution requirements.
- Use Markdown sections in this order: the problem description, **Examples**, then **Constraints**. Give 2-3 examples with explicit **Input:**, **Output:** and **Explanation:** lines. The app numbers examples, so do not number them in the statement. Put one checkable rule on each constraint bullet.
